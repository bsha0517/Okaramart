  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/categories", {
        ...
        body: JSON.stringify({ name, slug: slugify(name), imageUrl: imageUrl || undefined }),
      });
    ...
    <form onSubmit={handleSubmit} className="bg-white border border-canal/10 rounded-xl p-4 flex flex-wrap gap-3 items-end">
      ...
      <div>
        ...
        <input value={imageUrl} onChange={(e) => setImageUrl(e.target.value)}
          placeholder="https://…" className="border border-canal/20 rounded-lg px-3 py-2 text-sm w-64" />
      </div>
